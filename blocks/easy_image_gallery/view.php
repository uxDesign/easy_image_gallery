...
?>
    ...
    <?php foreach ($files as $key => $f) :
        ...
        <div class="box-wrap masonry-item b-col-<?php echo $imageColumn ?> <?php echo $tags ?>">
            <?php if($fullUrl) : ?><a href="<?php echo $fullUrl ?>" 
                <?php if ($options->lightbox) : ?> data-fancybox-group="easy-gallery-<?php echo $bID?>" data-image="<?php echo $fullUrl ?>" 
                    <?php if($options->lightboxTitle) : ?> title="<?php echo $f->getTitle() ?>
                        <?php if($options->lightboxDescription) : ?> 
